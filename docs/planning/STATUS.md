...
Issues: 212 en total, 198 cerrados, 14 abiertos
...
2. **La vault real va una iteración por detrás, y eso deja sin ejecutar una mitigación escrita.** El último despliegue fue #373, con el código de la Iteración 11. **La vuelta nueva del patrón**: no es una afirmación falsa ni una mitigación mal pensada, es una mitigación correcta que el calendario de despliegues hace inaplicable — y nada lo señalaba, porque cada iteración despliega la anterior y eso parece normal visto de cerca. No verificado contra la máquina, que estaba apagada al planificar (`No route to host`); se comprueba al desplegar (#412).
...
1. [#429](https://github.com/ecamp0s/evault/issues/429) fix(web): editar una entrada favorita la desmarca, y nada lo avisa (High)
...
| [#412](https://github.com/ecamp0s/evault/issues/412) | chore(ops): desplegar la Iteración 12 en kastor y etiquetar de verdad las 370 entradas | `chore` `s13` | Todo | High | #429 | #425 |
...
| [#416](https://github.com/ecamp0s/evault/issues/416) | feat(web): el campo del segundo factor en una entrada | `feat` `web` `s13` | Todo | High | #414, #415, #429 | #417, #419, #420, #425 |
...
| [#425](https://github.com/ecamp0s/evault/issues/425) | docs: cerrar la Iteración 13 | `s13` | Todo | High | #412, #413, #414, #415, #416, #417, #418, #419, #420, #421, #422, #423, #424, #427, #429 | — |
...
| [#429](https://github.com/ecamp0s/evault/issues/429) | fix(web): editar una entrada favorita la desmarca, y nada lo avisa | `bug` `web` `s13` | Todo | High | — | #412, #416, #425 |
...
| **Las etiquetas resultan no ser lo que hacía falta** | `Abierto` | Se eligen etiquetas y no carpetas con un argumento razonable —una entrada no tiene por qué estar en un solo sitio— pero **es un argumento y no una medida**: nadie ha organizado todavía estas 370 entradas de ninguna forma. El riesgo no es que las etiquetas sean peores, es darlas por buenas sin volver a mirar. La mitigación es de cierre y no de diseño: #384 pregunta explícitamente qué apareció **usando** lo construido que no estaba previsto, que es la lección con la que la 11 se cerró (#378, #384) |
