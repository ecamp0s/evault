...
Issues: 236 en total, 232 cerrados, 4 abiertos
...
<!-- manual:objetivo -->
**Iteración 14: en curso desde el 2 de septiembre de 2026.** Objetivo: *la vault se instala en el móvil y se lee sin red.*

Es la **cuarta iteración seguida que elige su objetivo en vez de heredarlo**, y puede hacerlo porque la 13 cerró con el backlog vacío —218 issues, 218 cerrados— y cero deuda anotada.

**Y elige funcionalidad de uso, no política — la decisión de planificación que más conviene dejar escrita.** Sobre la mesa había una iteración de red de seguridad: historial de contraseñas, papelera y caducidad de sesión, encadenada al 246 que la 13 dejó a medias. Se descartó como objetivo con un argumento explícito: **eVault no es un producto que deba cumplir normas externas, es una instancia personal con dos cuentas reales**, y el rigor de sus decisiones tiene que ser proporcionado a eso. La decisión no se tiró —está escrita entera en `ADR-018`— pero entra en vigor cuando se planifique, no ahora.

**Catorce issues, del #458 al #471**, en seis bloques. Bloque 0, la planificación: #458. Bloque 1, el caché cifrado: #459, #460, #461 y #462. Bloque 2, la aplicación instalable: #463, #464 y #465. Bloque 3, que se vea y no engañe: #466 y #467. Bloque 4, la verificación: #468, #469 y #470. Bloque 5, el cierre: #471.

**Lo que cambia de fondo.** Hoy, si kastor no responde, **no hay vault** — ni siquiera para leer una contraseña consultada hace diez minutos. Y desde el 26 de agosto de 2026 eso ya no le pasa solo a quien administra la máquina: la instancia tiene **dos cuentas reales**, y la segunda no puede diagnosticar por qué kastor calla ni arreglarlo. Solo observa que la aplicación no funciona.

**El dato que abarata la iteración entera, y está en `ADR-019` §1: desbloquear sin red no necesita servidor en absoluto.** Por `ADR-008`, el hash de autenticación solo sirve para conseguir un token y el token solo sirve para traer el ciphertext; si el ciphertext ya está en el dispositivo, no queda nada que pedirle a nadie. Y una contraseña incorrecta ya falla sola, porque produce una clave incorrecta y AES-GCM no valida su tag. No hace falta verificador aparte.

**El camino crítico no es el que sugiere el orden de los bloques: es #458 → #464 → #469.** El issue que comprueba que el caché sobrevive **siete días sin abrir la aplicación** no depende del caché ni del service worker — solo necesita que exista el manifest para poder instalar y empezar a contar. Por eso #464 va en `High` aunque poner iconos no suene urgente, y por eso #469 se empieza en cuanto exista el manifest. Si llega al final de la iteración, no cabe.

**Dos ADR nuevos, y con ellos son diecinueve.** `ADR-019` decide esta iteración: caché de solo lectura, opt-in por dispositivo, IndexedDB, y nada descifrado en disco. `ADR-018` queda `Aprobada, con entrada en vigor diferida`, que tiene precedente literal en `ADR-007` — estuvo diferido hasta la Iteración 3.

**Lo que `ADR-019` acepta en voz alta, porque es lo caro de deshacer:** cachear la vault **quita el rate limiting de en medio**. Adivinar la contraseña maestra contra kastor pasa por el limitador de la API; contra un caché local no pasa por nada. Lo que lo hace aceptable es que el proyecto ya asumió esa propiedad en la Iteración 4 con los ficheros `.evault`, y las 600.000 iteraciones de PBKDF2 se dimensionaron para eso. Cambia la frecuencia, no la naturaleza — y por eso el caché está **apagado por defecto**.
...
1. [#490](https://github.com/ecamp0s/evault/issues/490) feat(web): sin red, la aplicación tarda varios segundos en pintar (Medium)
...
| [#490](https://github.com/ecamp0s/evault/issues/490) | feat(web): sin red, la aplicación tarda varios segundos en pintar | `feat` `web` `s14` | Todo | Medium | #465 | — |
