...
Issues: 72 en total, 50 cerrados, 22 abiertos
...
1. [#114](https://github.com/ecamp0s/evault/issues/114) docs: planificar la Iteración 4 (High)
...
| [#114](https://github.com/ecamp0s/evault/issues/114) | docs: planificar la Iteración 4 | `s4` `chore` `documentation` | Todo | High | — | #120, #121 |
...
8. ⬜ **`master` protegido por ruleset, y el bot regenerando `STATUS.md` sin romperse** (#110, #21).
...
| `master` sin protección | `Abierto, ya mitigable` | **Dejó de ser imposible al hacerse público el repositorio**: GitHub no admite rulesets en repos privados de cuentas Free, pero sí en públicos. Entra en la Iteración 4 con #110, que cierra #21. La trampa a evitar está anotada allí: el workflow `status` escribe en `master`, así que un ruleset que exija pull request deja al bot sin poder regenerar este documento si no se le da bypass. El hook `pre-push` sigue cubriendo el despiste local |
| El repositorio es público con el escaneo de secretos desactivado | `Abierto, Iteración 4` | Comprobado contra la API el 3 de agosto de 2026: `secret_scanning` y `secret_scanning_push_protection` en `disabled`, sin descripción y sin topics. En un repositorio cuyo tema es la custodia de secretos, la *push protection* apagada es difícil de defender. Se resuelve en #110 |
<!-- /manual:riesgos -->
