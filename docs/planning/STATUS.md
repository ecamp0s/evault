...
1. ⬜ **La instancia personal sirve la vault por HTTPS y guarda contraseñas reales.** Verificado **desde otro dispositivo de la red**, no desde la máquina que la sirve: la excepción de `.localhost` vale en la máquina que ejecuta el navegador y no desde otro dispositivo, así que probarlo en kastor daría un falso verde (#224, #227).
...
7. ⬜ **Actualizar la instancia con datos reales dentro sin perder nada**, contando los items antes y después, e **incluida la vuelta atrás ejecutada de verdad** — que es la parte que nadie ejecuta nunca y por eso es la que estará rota (#226).
