...
El 323, la jubilación: borrar check-identifiers.py, english.txt, los dos extractores y sus tests —1.860 líneas—, quitar su job del workflow y sus menciones de CLAUDE.md y de este documento, y decidir por escrito qué pasa con dump-ui-text.mjs, que no vigila idioma y sigue teniendo trabajo. EN ESE MISMO PR va el paso del comprobador de idioma a --all: con esos cuatro ficheros fuera el árbol queda entero en inglés, así que el check entra en verde y no queda ningún momento sin red. Después quedan el 324 y el 325, y el 326 cierra la iteración.
