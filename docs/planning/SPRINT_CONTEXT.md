...
ADMIN.EVAULT.LOCALHOST NO ES UN PANEL: es un matcher que quedó en el Caddy de la máquina y que sirve la raíz del mismo proyecto Laravel que la API. Comprobado con curl, no leído. Se documenta como lo que es, un resto, en vez de retirarlo: el Caddy de desarrollo no está en el repositorio y da servicio además a otro proyecto en la misma máquina, así que se toca aparte y verificando que el otro sigue respondiendo.

Y AL COMPROBARLO APARECIÓ OTRA COSA, que es del mismo patrón y va al 342: api.evault.localhost RESPONDE 200 en esta máquina, aunque el 296 lo retirara, y app.evault.localhost/api NO llega a PHP-FPM sino al 5173. Es decir que el Caddy del sistema nunca recibió el cambio de ADR-016. No se nota trabajando porque con Vite levantado el proxy del servidor de desarrollo lo cubre. CLAUDE.md lleva ahora el comando que lo distingue: con Vite apagado, curl a /api/health responde 200 si Caddy lo enruta y 502 si no.
