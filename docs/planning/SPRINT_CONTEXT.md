...
El 229, que no se puede llegar a la vault desde fuera de la red local, y el 251, convertir a inglés los comentarios y los nombres de test que quedan en español.

Y una cosa del entorno que ahorra un rato al empezar: EL FRONTEND EXIGE NODE 24 Y DESDE EL ISSUE 255 SE COMPRUEBA AL INSTALAR. Si npm ci falla con EBADENGINE, la respuesta es actualizar Node y no tocar el .npmrc. Antes esa exigencia no se comprobaba y el incumplimiento aparecía disfrazado: con Node 20, jsdom 30 revienta con markAsUncloneable is not a function y la suite informa de cero tests ejecutados, sin mencionar Node por ninguna parte.

Del 251 hay que saber que LA REGLA DE IDIOMA CAMBIÓ EL 17 DE AGOSTO: la frontera pasa ahora entre ficheros —código en inglés incluidos comentarios y nombres de test, documentación en español— y no por dentro de cada fichero. Lo ya escrito se queda hasta esa conversión, así que conviven los dos idiomas a propósito. Y el comprobador de identificadores se retira CON la conversión y no antes: mientras haya prosa española pegada a código inglés, el riesgo que lo justifica sigue vivo y es lo único que lo detecta. Está dicho dentro del propio comando. El 230 y el 232 se cerraron en la propia iteración. Del 232 conviene saber lo que dejó escrito, porque vuelve a morder: LOS AVISOS DE SONNER NO VIVEN EN EL ÁRBOL DE REACT, su estado es global al módulo, así que cleanup() no los borra y se filtran entre tests. Lo cubre un toast.dismiss() en el afterEach global, con test propio. El 229 se deja fuera de la 7 a propósito, porque puede acabar resolviéndose con una instancia en hosting compartido en vez de con un túnel, y esa decisión no es de esta iteración. Las dos que dejó la Iteración 6 —el 197 y el 202— se cerraron después de ella.
