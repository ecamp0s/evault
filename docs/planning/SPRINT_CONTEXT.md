...
Por qué el proyecto está construido así: los veinte ADR en docs/architecture/decisions.
...
Los veinte ADR de docs/architecture/decisions son la fuente de verdad, y son inmutables: si una decisión cambia, se escribe uno nuevo que la supersede. Lo que sigue es el índice para saber cuál abrir, no un sustituto de abrirlo.
...
EL SEGUNDO FACTOR: AHORA SE ENTIENDE Y AUN ASÍ NO SE VA A USAR, y esa distinción es todo el valor del 545. Antes no se podía separar «no lo quieren» de «no se lo han explicado», que son conclusiones opuestas con la misma apariencia. Se explicó —qué es, para qué sirve y qué se paga— y la respuesta fue: «ahora entiendo, me parece interesante, pero no lo veo en el medio plazo; si más adelante lo necesito lo retomamos». O sea NO SE RETIRA, y la decisión de no retirarlo está tomada CON el dato y no sobre la ambigüedad.

LO QUE ESO DEJA PARA LA ITERACIÓN 13, y hay que decirlo: su criterio 2 —un código de eVault aceptado por un servicio real— NO SE VA A VERIFICAR, porque nadie va a usar la funcionalidad. Deja de ser una tarea pendiente y pasa a ser una propiedad conocida.

LOS CAMPOS DEL BLOB YA ESTÁN EN INGLÉS: name, username, notes, favourite, tags, type, cardholder, number, expiry, y los valores card y note. password, url, totp, csc y pin ya lo estaban. NO SE ESCRIBIÓ ADR y version NO SUBE, y las dos cosas tienen motivo: la decisión que cambió es la regla de idioma, que vive en CLAUDE.md y no en un ADR; y version es la del esquema criptográfico, que no se ha tocado —además, tras el vaciado no queda contenido viejo del que distinguirse—. ADR-020 sección 5 sigue escribiendo los campos en español y NO se corrige, porque los ADR son inmutables, igual que ADR-007 y ADR-019 nombran claves que el 476 renombró.

Y LO QUE ESTE RENOMBRADO ENSEÑA SOBRE LA HERRAMIENTA: ui-text.mjs cazó CINCO frases españolas rotas que el compilador y los 922 tests no vieron —«Solo el name es obligatorio», «Se busca por name, username…», «tu name de username»…—. Una de ellas, «nombre y no se puede importar», estaba PARTIDA POR UNA INTERPOLACIÓN en JSX y quedaba sola en su línea: es el fallo del 115 repetido, y es la razón exacta de que esa comparación exista. Un renombrado se verifica comparando el texto visible entero, no leyendo el diff.

LA REGLA DE IDIOMA YA NO TIENE EXCEPCIONES POR MIEDO A PERDER DATOS: el código va entero en inglés, y si un renombrado obliga a migración o a reset SE AVISA DEL IMPACTO Y DECIDE QUIEN TIENE LA VAULT. De las cinco excepciones que había en CLAUDE.md, TRES ERAN FALSAS —barridas una a una antes de borrarlas— y una de ellas NUNCA FUE CIERTA: la clave del state de react-router es «from», nunca estuvo en español.

Y LO QUE LAS HACÍA DAÑINAS NO ERA ESTAR OBSOLETAS SINO QUE FABRICABAN ESPAÑOL NUEVO: tipo, titular, numero, caducidad y los valores tarjeta y nota nacieron en español el 8 de septiembre en el 520, UN DÍA ANTES de la decisión, porque la lista decía que los campos del blob van en español. Quedan dos excepciones ciertas —los campos del blob y una migración aplicada— y ahora llevan SU SALIDA ESCRITA, que es lo que faltaba: se renombran con migración en el cliente o vaciando, no es que no se puedan renombrar.

LA TARJETA Y LA NOTA ESTÁN EN LA VAULT REAL Y SE LEEN EN EL IPHONE, y de las comprobaciones que solo se podían hacer allí salieron dos respuestas: el teclado numérico SÍ sale, y iOS NO OFRECE NADA sobre el campo del número —la franja de encima del teclado se queda vacía—. Esa segunda convierte en medida lo que el comentario de CardFields.tsx afirmaba sin haberlo comprobado, y era el sitio donde más probable era equivocarse.

Y APARECIÓ UN FALLO DE MODELO MENTAL QUE TIENE ISSUE, EL 546: el caché NO es «por dispositivo» como dicen la pantalla y ADR-019, sino POR CONTENEDOR DE ALMACENAMIENTO. En iOS, Safari, Chrome y cada app instalada están separados. Le ocurrió a quien tiene la vault: lo activó en Chrome y creía que su iPhone guardaba copia; la app instalada no guarda ninguna. La pantalla no miente sobre su estado —diría bien que no hay copia—, pero la palabra «dispositivo» hace innecesario ir a mirarlo.

LA HUELLA CON LA QUE SE VERIFICA UN DESPLIEGUE ESTABA ROTA, Y ESO HAY QUE TENERLO PRESENTE AL DESPLEGAR: DEPLOYMENT.md sección 7 usaba SHA2(GROUP_CONCAT(ciphertext)) a secas, y GROUP_CONCAT trunca a group_concat_max_len —1024 en kastor— EN SILENCIO. Medido: cubría 1024 de 120300 bytes, el 0,85 por ciento, unas dos filas de 639. El comando corregido levanta el límite, ordena por id e IMPRIME LA LONGITUD al lado, que es lo que delata el truncamiento la próxima vez.

SE DESCUBRIÓ PORQUE EL NÚMERO NO CUADRABA y no porque nadie lo auditara: se habían añadido dos entradas entre un despliegue y el siguiente y la huella salió LA MISMA. Es la familia del PerformanceObserver de largeVault.mjs y del grep sin -a del 184 — un número tranquilizador que no mide lo que dice—, esta vez en el comando del momento de más riesgo del proyecto.

LOS CUATRO CAMPOS NUMÉRICOS DE LA TARJETA PIDEN TECLADO NUMÉRICO —inputMode y NO type=number, que traería consigo el rechazo silencioso de lo que el navegador no considera un número, es decir ADR-020 sección 6 roto por la elección de un tipo de input—. Y la caducidad escribe la barra sola.

LA REGLA QUE HACE COMPATIBLE ESA MÁSCARA CON ADR-020 SECCIÓN 6: una máscara que AYUDA a escribir no es una validación que RECHAZA. Solo actúa sobre una tirada de dígitos pelada; en cuanto el valor lleva cualquier otra cosa se devuelve intacto, que es además lo que evita que pelee con el retroceso. Comprobado en navegador: 0924 da 09/24, dos retrocesos dan 09/ y «mayo 2029», «09/2024» y «09-24» se guardan tal cual.

EL NÚMERO DE LA TARJETA ABRE VISIBLE, y el código de seguridad y el PIN no. Salió de usar la aplicación con una tarjeta real. El argumento que lo tenía oculto —lo que no está en el DOM no lo lee nadie— es de LA LISTA, donde el número sigue sin pintarse; en el editor el valor está en el DOM igual y los puntos solo tapan a quien mire por encima del hombro. A cambio se pagaba lo que de verdad se usa: EL NÚMERO ES EL CAMPO QUE SE LEE, no el que se copia —quince o dieciséis dígitos para teclear donde no dejan pegar, o para cotejar con el plástico—. La línea queda en lo que POR SÍ SOLO completa un pago o abre un cajero. Las demás garantías no se tocan: sigue sin pintarse en la lista, sin indexarse y copiándose con limpieza del portapapeles.

LA VAULT SEMBRADA YA LLEVA LOS TRES TIPOS, en un ciclo de doce: una tarjeta, una nota y diez logins. Sobre 370 salen 31, 31 y 308. La proporción de tipos está ELEGIDA Y NO MEDIDA —no hay ninguna vault con tarjetas que medir— y lo que la fija es lo que la medición necesita: por debajo de una de cada doce, una pantalla de diecinueve filas puede no contener ninguna, y las variantes de fila quedarían sin ejercitar justo en la ejecución que vigila el DOM.

Y LO QUE HAY QUE SABER SI ALGUIEN TOCA ESA SIEMBRA: las dos de cada tres contraseñas malas se cuentan SOBRE LOS LOGINS y no sobre las entradas, que es para lo que existe loginIndex. Las dos ranuras del ciclo son múltiplos de tres, así que dejarlo en i % 3 habría quitado del recuento solo contraseñas BUENAS y la proporción se habría ido de 0,666 a 0,799 —calculado, no temido—. La revisión marca ahora 205 de 308 y los ocho límites siguen en verde.

EL IMPORT ENTIENDE LOS TIPOS EN EL FORMATO NATIVO —el .evault devuelve las tres formas idénticas, clave desconocida incluida— Y NO LOS INVENTA DESDE UN CSV AJENO. Ese segundo caso NO es teórico: el fichero de Bitwarden TIENE una columna type, con los valores login, note y card en inglés, y leerla como la nuestra sería un cambio de una línea que parece una mejora. No lo es por dos motivos: esas palabras no son nuestros valores, y type: login escribiría una clave que ADR-020 sección 4 exige AUSENTE.

Y APARECIÓ UN HALLAZGO QUE NO ES DE ESE ISSUE Y TIENE EL SUYO, EL 531: reimportar NUESTRO PROPIO CSV en claro devuelve la tarjeta como login Y MANDA EL NÚMERO A notas, que es el único campo que la búsqueda indexa. Nuestro CSV tiene la firma de Chrome, así que se detecta como Chrome. No es silencioso —informa de las columnas movidas— pero informar de que se ha degradado un secreto no es lo mismo que no degradarlo.

EL EXPORT YA SE LLEVA LOS TIPOS, y de ahí salió un aviso que había dejado de ser cierto: la confirmación del CSV en claro prometía un fichero con todas las contraseñas legibles, y desde ADR-020 ese fichero puede llevar además números de tarjeta, códigos de seguridad y PIN. Un aviso que enumera MENOS de lo que hay dentro es de los que se leen, se aceptan y se entienden tarde. Ahora cuenta las tarjetas y lo dice antes de descargar, y CALLA cuando no hay ninguna: avisar de algo que el fichero no contiene es otra forma de estar equivocado.

LA BÚSQUEDA YA ENCUENTRA AL TITULAR, y solo al titular: de la tarjeta entra ESE campo y ninguno más. Fuera el número, el csc y el pin por secretos, y fuera la caducidad porque no distingue nada. Y FUERA TAMBIÉN LOS CUATRO ÚLTIMOS DÍGITOS, que es lo único que hubo que decidir en vez de deducir: son la forma cómoda de elegir tarjeta en una pantalla de pago Y exactamente lo que pide un banco por teléfono para identificarte. La nota ya se encontraba por su cuerpo, porque notas se indexa desde siempre.
...
LA TARJETA YA SE EDITA, con sus cinco campos en el orden en que están impresos y tres de ellos tratados como contraseñas: número, código de seguridad y PIN. La etiqueta del tercero es «Código de seguridad» y NO dice cuántos dígitos tiene ni dónde está impreso, porque cualquiera de las dos ayudas sería falsa para una marca: los de American Express son CUATRO y van al anverso.

Y DE AHÍ SALIÓ UN FALLO VIVO EN MASTER QUE NO ERA DE ESTA ITERACIÓN: copySecret y copyValue recibían un sustantivo y le pegaban un participio FIJO —copiada en uno, copiado en el otro—, así que la frase solo era correcta mientras cada llamante eligiera una palabra del género que tocaba. Dos de las cinco no lo hacían, y la aplicación decía «Código copiada» y «Clave copiado». Ahora el llamante pasa la frase entera; el género lo sabe quien elige la palabra, no la plantilla. Lo mismo con los rótulos accesibles: se les pasa «el número» y no «Número».

EL SELECTOR DE TIPO YA ESTÁ, y lo que decide sin que se vea en el código: ABRE YA ELEGIDO EN «Inicio de sesión», porque 370 de las 370 entradas de la vault real lo son y poner una pregunta delante de la acción más frecuente le cobra a todo el mundo lo que casi nadie hace. Solo aparece al CREAR: al editar no hay selector y quien dice de qué tipo es la entrada es el título —«Editar tarjeta», «Editar nota»—. Y NO LLEVA AVISO de que el tipo no se puede cambiar, a propósito: la lección del 498 es que un coste enunciado sin un «entonces haz esto» deja a quien lee sosteniendo una alarma sobre la que no puede actuar; lo que hace revisable la elección es que se queda en pantalla mientras se rellena.

LA TRADUCCIÓN ENTRE EL BLOB Y LA PANTALLA VIVE EN UN SOLO SITIO, lib/vault/itemTypes.ts. En el blob un login NO TIENE tipo; en pantalla no existe la opción ausente, porque quien elige está eligiendo entre tres cosas con nombre. Modelar la elección como ItemContent['tipo'] habría metido ese undefined en cada componente para que significara «login» por convención en todos.
...
LA MEDIDA EN VUELO: el 469 vence el MIÉRCOLES 9 de septiembre de 2026 a las 17:43, y NO se abre la PWA del iPhone antes de esa hora. Hacerlo no adelanta el resultado, lo destruye. Mide si Safari poda el almacenamiento de una aplicación instalada, que es lo que decide si el offline existe de verdad en el iPhone. Es la única medida con reloj y no bloquea nada de la 15.

LO QUE QUEDA PENDIENTE Y TIENE DUEÑO CONOCIDO, y sigue igual: probar un código TOTP contra un servicio real, que es el criterio 2 de la 13 y necesita una cuenta de prueba y una persona; y bajar el recuento de la auditoría, que sigue en 246 de 369 con una contraseña compartida por 41 entradas. Las dos son trabajo de quien tiene la vault y no del repositorio, así que se anotan como medición y no como criterio. Y OJO al leer ese 369 a partir de ahora: con notas y tarjetas dentro deja de ser el número de entradas y pasa a ser el de entradas CON contraseña.

EL CANDIDATO DE LA 16 YA ESTÁ ELEGIDO Y TIENE SU BLOQUEO DELANTE: la extensión de Firefox y Chrome, que es donde está el autofill. No puede empezar sin resolver DÓNDE VIVE LA CLAVE DESBLOQUEADA, porque Manifest V3 mata el service worker de fondo y eso choca de frente con ADR-007. Es un ADR antes que un issue, y el resultado del 469 le sirve: es la misma pregunta desde otro sitio, cuánto te puedes fiar de que el navegador conserve algo.

Y APARECIÓ UN TERCER CANDIDATO AL PLANIFICAR LA 15, que no estaba en ninguna lista: las VAULTS COMPARTIDAS. ADR-008 lleva anticipándolas desde la Iteración 3 —la clave envuelta vive en vault_members y no en vaults justamente para no rehacerlo el día que haya dos miembros—, y lo único que falta es criptografía asimétrica, que ADR-001 dejó para más adelante. Hay dos cuentas reales en la instancia, así que hay caso de uso. Es el que mejor demuestra el modelo, y también el más caro y el más peligroso: toca la arquitectura de claves sobre una instancia con 370 contraseñas que el servidor no puede reparar. Y ADR-008 sección 6 ya dice que compartir es una de las tres ocasiones naturales de revisar esa arquitectura.

LO QUE SIGUE SOBRE LA MESA Y NO ES CANDIDATO INMEDIATO: una app nativa de iOS como proveedor de contraseñas, que NO se puede hacer con una PWA y es un cliente entero y no una funcionalidad; leer una semilla desde un código QR, descartado en la 13; y ejecutar ADR-018 —historial, papelera y caducidad de sesión—, aprobado y diferido con motivo escrito.

LO QUE NO HAY QUE REABRIR POR INERCIA: paginar GET /items, descartado con la medida delante en la 11; el acceso desde fuera de la red local, resuelto; el hosting compartido como vía de acceso, descartado en ADR-015; el panel Filament, fuera de alcance por ADR-009 sección 4; las carpetas, descartadas CON MEDIDA en la 13; el offline con escritura y cola de sincronización, descartado en ADR-019 porque el servidor no puede resolver un conflicto que no puede leer; y los DOCUMENTOS ADJUNTOS dentro del blob, descartados en ADR-020 sección 7 porque GET /items devuelve todos los items sin paginar y un adjunto se descargaría entero en cada carga.

Y LO QUE SE MIRA SIN QUE SEA UNA TAREA: las tres señales del hosting compartido como emplazamiento, con el disparador de ADR-013 sección 6 — teniendo en cuenta que la Iteración 14 DESACTIVÓ EN PARTE la primera de ellas, según ADR-019 sección 6.3: ya no hace falta que kastor esté encendido para consultar la vault en un dispositivo que guarde copia.
