...
La Iteración 4 no está planificada del todo, pero el cambio de rumbo de arriba ya le ha fijado el núcleo: dejar el proyecto en condiciones de ser público y de ser usado de verdad por su autor. El issue 103 fue el primero de esa línea y cubrió el README en inglés, la licencia MIT y el arranque reproducible en un clon. El ADR-009 registró después el cambio de rumbo. Y el 107 salió de probar ese arranque en un clon de verdad: funcionaba, pero saltarse la copia del .env producía una página en blanco muda, así que ahora el servidor de desarrollo se niega a arrancar sin VITE_API_URL y el index.html deja un aviso dentro de root que React reemplaza al montar. La lección, que vale para lo que venga: lo que hay que probar no es el camino feliz sino el paso que alguien se salta.

Lo que queda sobre la mesa, en el orden en que tiene sentido tomarlo. Export cifrado de la vault y backup automático, que van antes de meter ninguna contraseña real y hoy no existen en ninguna forma. Docker compose, que sirve a los tres propósitos a la vez: el clon ajeno, la demo y la instancia personal. El ADR que registra este cambio de rumbo. La demo pública con cuentas efímeras, diseñada ya y pendiente de trocear en issues. Cambio de contraseña maestra, que ADR-008 abarató a reenvolver un blob. Y clave de recuperación, que ADR-001 dejó apuntada como mitigación de que no haya recuperación. Aparte, la deuda de arriba. El repositorio ya es público desde el 3 de agosto de 2026 y pasó a llamarse evault, sin el sufijo. Eso desbloquea el issue 21, porque GitHub sí admite rulesets en repos públicos de cuentas Free, con una trampa que está anotada en el issue 110: el workflow status escribe en master, así que un ruleset que exija pull request deja al bot sin poder regenerar STATUS.md si no se le da bypass. El historial se auditó antes de publicar y está limpio, nunca hubo un .env versionado.
