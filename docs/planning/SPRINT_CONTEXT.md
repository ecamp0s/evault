...
EL 424 SACÓ /styleguide DEL ARTEFACTO DE PRODUCCIÓN: la ruta ya estaba tras import.meta.env.DEV, pero el import() estaba fuera, así que Rollup emitía su chunk en cada build. Nunca se descargaba —la ruta no está registrada—, y por eso el motivo no era el tamaño sino que una pantalla de desarrollo viajaba dentro de lo que sirve la máquina con las contraseñas reales. Y conviene no leer mal el log del build: el chunk principal baja de 343 kB a 256, pero eso es Rollup partiendo distinto, no código que se va — sobre dist/assets entero son 7,5 kB crudos y 3,5 gzip de 890.

EL 442 CIERRA LA OTRA MITAD DE ADR-011 §2.4: la previsualización del import señala ahora las filas repetidas DENTRO DEL PROPIO FICHERO y no solo contra la vault. Esa sección pide marcar las que PARECEN repetidas, y dos filas iguales en un mismo fichero lo parecen tanto como una que choca con algo guardado. Sobrevive la primera y se marcan las siguientes, para no obligar a desmarcar cada copia. Firefox es lo que lo hace probable: sin columna de nombre, todo lo de un servicio colapsa en el mismo nombre derivado del host y solo queda el usuario para distinguir.

EL 423 CERRÓ EL BANCO CON OCHO LÍMITES, y el nuevo es el que encontró el 450: cuánto multiplica la página abrir la revisión. Trae RECIBO —se niega a pasar si la pantalla no auditó nada ni pintó filas—, porque su primera versión salió verde sobre una revisión vacía: las contraseñas sembradas eran todas buenas y un ×0.1 solo decía que una página vacía es pequeña. La vault del banco tiene ahora dos de cada tres contraseñas malas, que es la proporción de la real.

Y HUBO UN SEGUNDO CERO TRANQUILIZADOR EN EL MISMO SITIO: el regex que lee el titular iba dentro de un template literal, donde \d pierde la barra y acaba buscando letras d. Decía «0 de 0» sobre una pantalla que estaba pintando 120 filas — un número equivocado, que es más difícil de ver que uno ausente.

Y EL 452 ARREGLÓ UN TEST QUE ESCRIBÍ INTERMITENTE: TotpCode esperaba una cantidad FIJA de reloj a que llegara el código siguiente, y generar uno pasa por crypto.subtle, así que cuánto tarda depende de la máquina. Verde aquí, rojo en el CI, que tiene dos núcleos —donde nada de esto se midió, que es la nota del 259—. Ahora espera a la condición y no al tiempo. NO SE PUDO REPRODUCIR EN LOCAL ni forzando dos núcleos con taskset y carga encima: diez pasadas en verde con el código roto. Así que lo que verifica esto es el CI y no una mutación, y conviene saberlo antes de fiarse.

Y EL 450 LA ACOTÓ, PORQUE DESHACÍA LO QUE LA ITERACIÓN 11 COMPRÓ. El límite nuevo del 423, corrido a 370 entradas con la proporción real de marcadas, la pilló pintando 738 filas y 4028 nodos contra los 557 de la lista: ×7.2 sobre un margen de ×3. No crece con la vault sino con LO QUE ESTÁ MAL dentro de ella, y por eso a 120 entradas daba ×2.5 y parecía bien — extrapolé y me equivoqué. Cada sección pinta ahora veinte filas y ofrece el resto: ×0.7, o sea que la revisión cuesta menos que la lista. Y las repetidas van ordenadas por cuántas comparten, porque si solo se ven veinte importa cuáles: en la vault real hay un grupo de 41.
...
EL 419 CIERRA EL BLOQUE DE TOTP: el import mapea login_totp de Bitwarden a su campo en vez de arrastrarlo a las notas, que es lo que ADR-017 §4 pedía por su nombre — y lo que importaba no era el orden sino que NOTAS ES LO QUE LEE LA BÚSQUEDA. Bitwarden es el único formato aceptado que trae esa columna: Chrome y Firefox no la tienen, y el CSV propio retiene la semilla por diseño.

Y CON UN MATIZ QUE NO ESTABA PREVISTO Y CONVIENE NO PERDER: una semilla que NO se puede leer sigue yendo a las notas. Escribirla en el campo daría códigos plausibles que nadie acepta, y descartarla sería perderla en silencio, que es lo que ADR-011 §2.4 llama el peor fallo posible. Es un mal menor asumido a conciencia —un valor que quería ser semilla acaba en un campo indexado— y no un descuido. Lo decide parseTotp y no un regex, porque una base32 con la forma correcta puede decodificar a los bytes equivocados.

EL 418 CIERRA LA CONSECUENCIA 4 DE ADR-017: un reloj desviado produce códigos correctos para un instante que no es ahora, y el síntoma es «eVault está roto». Se mide leyendo la cabecera Date de respuestas que iban a llegar igual —ni endpoint, ni petición extra, ni cambio de contrato, y al servidor no se le pregunta nada—, y el aviso sale DONDE SE VE EL CÓDIGO, nombrando la causa. EL CÓDIGO SE SIGUE GENERANDO CON EL RELOJ LOCAL, nunca con la hora del servidor: hacerlo ataría los códigos a que haya red, cuando TOTP existe para no necesitarla, y escondería un reloj mal puesto que rompe otras cosas.

EL UMBRAL ES UN PASO ENTERO, 30 SEGUNDOS, y no está elegido a ojo: los servicios aceptan casi siempre el código anterior y el siguiente, así que por debajo de un paso el desvío lo absorbe esa tolerancia. Avisar antes sería avisar cuando no pasa nada, y un aviso que grita sin motivo se ignora junto con los que importan.
...
Y VERIFICANDO ESO EN EL IPHONE SALIÓ EL 437, que es el hallazgo de la sesión: con el diálogo de una entrada abierto NO SE LLEGA AL BOTÓN DE GUARDAR. El diálogo se centra con -translate-y-1/2 y Base UI bloquea el scroll del body, así que lo que sobra se reparte arriba y abajo y no hay gesto que lo alcance. Medido a 664 px de alto: el diálogo medía 844 con top -90. Y el fallo YA EXISTÍA antes del campo de segundo factor —desbordaba 57 px— pero el 416 lo triplicó hasta 180 y lo pasó de incómodo a imposible. Arreglado con max-h en dvh y overflow-y-auto en DialogContent, que es de components/ui y por tanto lo regenera el CLI de shadcn: la corrección lleva su motivo comentado dentro para que una regeneración no se la lleve en silencio.

Y AL ETIQUETAR DE VERDAD SALIÓ EL 439, más pequeño pero de la misma familia: la fila de chips se pegaba a la primera entrada sin nada de aire —0 px medidos, contra los 8 que hay entre dos tarjetas—, y desde el móvil se lee como si se solaparan. El margen va DENTRO de TagFilter y no en el contenedor, porque el componente devuelve null cuando no hay etiquetas y un margen fuera dejaría un hueco fantasma en toda vault sin etiquetar.
