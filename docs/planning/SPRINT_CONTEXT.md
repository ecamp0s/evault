...
LAS DOS AFIRMACIONES CADUCADAS, RETIRADAS EN EL 324. La primera: CLAUDE.md y SETUP.md prometían un panel Filament que ADR-009 sección 4 había sacado del alcance, y que no está ni instalado. ADR-002 no queda contradicho y no se toca: decidió que SI hubiera panel sería Filament y no React, y eso sigue siendo verdad; lo que ADR-009 retiró fue el sujeto. La segunda: el hook pre-push decía en su cabecera que el issue 21 estaba bloqueado porque GitHub no permitiría rulesets, y el 21 se cerró el 3 de agosto de 2026. Ahora la cabecera dice el reparto real —el ruleset del servidor cubre lo irreversible, el hook cubre el despiste de pushear a master, y el ruleset no exige PR porque el workflow de STATUS.md moriría con GH013—, y los cuatro caminos del hook se ejecutaron uno a uno para comprobar que sigue rechazando lo que tiene que rechazar.

Y EL COMPROBADOR DE IDIOMA TENÍA UN PUNTO CIEGO, que se vio al ir a reescribir ese hook: scripts/hooks/pre-push NO TIENE EXTENSIÓN, y check-comment-language.py decidía por la extensión. Es decir que el fichero llevaba veinte líneas de comentario en español mientras --all decía «sin problemas en el árbol entero». Un cero tranquilizador sobre un fichero que el auditor no abre es exactamente el 184. Ahora mira también los ejecutables sin extensión, identificándolos por el shebang, y con eso entran ese hook y api/artisan y nada más.

ADMIN.EVAULT.LOCALHOST NO ERA UN PANEL: era un matcher que quedó en el Caddy de la máquina y que servía la raíz del mismo proyecto Laravel que la API. Comprobado con curl, no leído. Se retira de los documentos y del Caddy, junto con api.evault.localhost, que el 296 había dejado sin sujeto y seguía respondiendo 200.

Y AL COMPROBARLO APARECIÓ OTRA COSA, que es del mismo patrón y va al 342: api.evault.localhost RESPONDÍA 200 en esta máquina, aunque el 296 lo retirara, y app.evault.localhost/api NO llegaba a PHP-FPM sino al 5173. Es decir que el Caddy del sistema nunca recibió el cambio de ADR-016. No se notaba trabajando porque con Vite levantado el proxy del servidor de desarrollo lo cubre. CLAUDE.md lleva ahora el comando que lo distingue: con Vite apagado, curl a /api/health responde 200 si Caddy lo enruta y 502 si no.

Y EL BLOQUE DE CADDY QUE HACE FALTA ESTÁ AHORA ESCRITO EN SETUP.md, que es lo que impide que esto vuelva a pasar: ese fichero vive en /etc/caddy, no está en el repositorio y necesita sudo, así que nadie podía reproducirlo leyendo el código y la única copia estaba en una máquina. YA APLICADO Y VERIFICADO el 21 de agosto de 2026, y el 342 se cerró con la medición delante: app.evault.localhost/api/health responde 200 con Vite apagado, /api/vaults sin token responde 401 —o sea que Laravel resuelve de verdad—, y ebudget y los hosts de codex siguen igual. El entorno de desarrollo se comporta ya como kastor. Ojo con una cosa al comprobarlo: un host retirado sigue respondiendo 200, con cero bytes, porque entra en el bloque :8080 y no casa ningún handle. Lo que distingue los dos casos es el tamaño del cuerpo.
...
El 342, que el Caddy de la máquina de desarrollo nunca recibió el cambio de ADR-016: api.evault.localhost sigue respondiendo 200 aunque el 296 lo retirara, y app.evault.localhost/api va al 5173 en vez de a PHP-FPM. No se nota trabajando porque el proxy de Vite lo tapa. Ese fichero no está en el repositorio, necesita sudo y sirve además a otro proyecto, así que se toca aparte y verificando los dos.
