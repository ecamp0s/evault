...
DEL 466: el aviso de «sin conexión» NO usa navigator.onLine, y es deliberado — dice si el dispositivo está enganchado a una red, no si kastor responde, así que un portal cautivo o una tailnet caída se leen como «online». Volver a conectar es una acción explícita que bloquea la vault, porque una sesión offline no tiene token y aquí no se guarda nada que pueda pedir uno.
