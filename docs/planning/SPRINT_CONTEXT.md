...
El issue 181, la cuarta capa del renombrado y la más grande: pages/vault, con 82 identificadores en 9 ficheros. Es la que concentra ImportDialog y ExportDialog, los dos ficheros con más texto de usuario del proyecto, así que la comparación del texto visible con node scripts/identifiers/dump-ui-text.mjs no es una formalidad: es la comprobación principal. La capa se mide sola con ./scripts/check-identifiers.py --area web.
