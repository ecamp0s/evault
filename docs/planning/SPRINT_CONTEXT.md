...
Estado: Iteración 15 cerrada el 9 de septiembre de 2026. La 16 sin planificar; los candidatos y la deuda abierta están al final, en SIGUIENTE PASO.
...
PLANIFICAR LA ITERACIÓN 16. El backlog está vacío salvo tres issues de deuda, y ninguno bloquea nada.

LA DEUDA ANOTADA, que es lo primero que hay que mirar porque las tres salieron de usar la aplicación de verdad. El 531: reimportar nuestro propio CSV en claro devuelve la tarjeta como login Y manda el número a notas, que es el único campo que la búsqueda indexa — un secreto degradado por la puerta de atrás. El 546: el caché no es «por dispositivo» como dicen la pantalla y ADR-019 sino por contenedor de almacenamiento, y en iOS Safari, Chrome y cada app instalada son sitios distintos; le pasó a quien tiene la vault. Y el 550: el botón que borra la copia de un dispositivo no parece un botón, y es el único que hace eso.

EL CANDIDATO QUE LLEVA DOS ITERACIONES ESPERANDO: la extensión de Firefox y Chrome, que es donde está el autofill. Sigue sin poder empezar hasta resolver DÓNDE VIVE LA CLAVE DESBLOQUEADA, porque Manifest V3 mata el service worker de fondo y eso choca de frente con ADR-007. Es un ADR antes que un issue. Y el dato del 469 ya está: Safari NO podó el almacenamiento de una aplicación instalada tras siete días, aunque lo que se midió fue el almacén propio de la app y no el caché de la vault.
