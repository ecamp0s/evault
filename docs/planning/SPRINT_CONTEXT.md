...
Y EL PRIMERO LO ENCONTRÓ QUIEN USA LA VAULT, NO UNA HERRAMIENTA, que es la lección de método de esta iteración: con 370 entradas el menú de usuario quedaba a 27.464 píxeles con la ventana en 900, así que para cerrar sesión, cambiar la contraseña maestra o llegar a la clave de recuperación había que recorrer las 370 entradas enteras. El aside tomaba la altura del documento y no la de la ventana. La suite pasaba, el análisis estático pasaba y el fichero no tenía nada raro al leerlo. ARREGLADO EN EL 350 con h-svh y sticky, y comprobado por mutación: al quitar esas clases el banco vuelve a rojo. Es otra vuelta de la lección de la Iteración 5, el camino que nadie recorre es el que está roto, y el camino era la vault real.
...
EL BANCO DE PRUEBAS DEL 348 YA ESTÁ, y era lo primero por la lección del censo del 316: nada de lo que esta iteración arregla lo detecta la suite —los tests de la lista montan tres items— ni se ve en un diff, así que sin un comando que mida, cada arreglo se daría por bueno porque se ve más rápido. Es node scripts/verify-large-vault.mjs, y no lo ejecuta el CI porque conduce un navegador.

NACIÓ EN ROJO SOBRE MASTER, que es la mitad que da sentido a la otra: sus seis límites por encima del margen, con el menú de usuario a 27.464 píxeles, 7.839 nodos de DOM contra 279 de una vault de diez, 740 peticiones para importar 370 entradas y dos para borrar una. Un banco que naciera verde sobre el código que se escribió para medir no estaría midiendo nada.

Y LO QUE DECIDE SON LOS RECUENTOS, NO LOS RELOJES, que es la decisión de diseño que hay que conocer antes de fiarse de un verde. Un umbral en milisegundos medido en un portátil sale rojo en otro sin que nada esté peor, así que los tiempos se comparan contra la misma vault de diez entradas en la misma ejecución y por sí solos no tumban nada. La propia ejecución lo demostró: el import tardó 60 segundos ahí contra los 4 minutos y 19 medidos a mano unas horas antes en la misma máquina con otra carga, mientras el recuento de peticiones salía idéntico.

LA LISTA YA ESTÁ VIRTUALIZADA, en el 349. Con las 370 entradas el DOM pasó de 7.839 nodos a 487 —una vault de diez tiene 289, así que ya no crece con lo que hay dentro—, pintar bajó de 668 a 168 milisegundos y buscar de 272 a 52. Pinta entre 19 y 30 filas según la posición del scroll. Entra la primera dependencia nueva del cliente en varias iteraciones, @tanstack/react-virtual, que son dos paquetes sin dependencias de terceros.

Y DE ESO SALIERON TRES COSAS QUE NO SE DEDUCEN LEYENDO EL CÓDIGO. Las filas NO miden todas lo mismo —70 píxeles sin usuario y 74 con él, medido—, así que la estimación va deliberadamente por lo alto: quedarse corto encoge la página y deja la última entrada sin alcanzar, comprobado con 46 píxeles fuera de la ventana. El lint de refs no era estilo sino un fallo real, porque leer el ref durante el render dejaba el scrollMargin en cero para siempre. Y jsdom no hace layout, así que la virtualización NO se puede verificar de verdad en la suite: allí pinta 159 filas de 300 y empieza por la 141. Lo que la verifica es el banco del 348 en navegador.

Y LA ESCRITURA YA NO RECARGA LA VAULT ENTERA, en el 352 y el 354, que eran el mismo defecto visto dos veces. Las tres mutaciones invalidaban la lista, y aquí invalidar no es una petición: baja la vault entera y la descifra, porque el servidor no puede mandar un diff de lo que no puede leer. Ahora se aplica a la lista en caché lo que el servidor acaba de confirmar. Borrar una entrada pasa de 2 peticiones y 437 milisegundos a UNA y 142; importar 370, de 740 peticiones y 35,7 segundos a 370 y 15,8.

CON ESO EL BANCO DEL 348 SALE ENTERO EN VERDE, sus seis límites, y con código de salida 0 por primera vez. Nació en rojo con los seis a propósito.

LO QUE NO ES OPTIMIZACIÓN SINO DECISIÓN, y hay que conocerlo antes de fiarse de la lista: además de actualizar la caché se marca la lista como stale con refetchType none, de modo que no cuesta ninguna petición ahora y la siguiente vez que la pantalla se monta se le pregunta al servidor quién tiene razón. Sin eso la vault sería tan correcta como lo último tecleado en ESTE dispositivo, y desde la Iteración 9 hay un móvil con la misma vault abierta. Verificado en navegador con dos pestañas: lo borrado en una desaparece de la otra al volver a la lista pasados los treinta segundos de frescura. NO es instantáneo, y decirlo importa más que la cifra.

Y EL 329 ESTÁ CERRADO: un bloqueo por inactividad con la clave de recuperación en pantalla ya no se la lleva en silencio. El aviso la nombra y dice qué hacer, en vez de hablar de un borrador perdido, y eso importa porque cuando la clave se ve YA ESTÁ REGISTRADA en el servidor: perder la pantalla deja una cuenta que dice tener una clave que nadie guardó. Verificado en navegador con reloj real, caso 8 de verify-auto-lock, 7 de 7 en verde en 18,4 minutos.

LO QUE EL 329 DABA POR MENOR Y NO LO ERA: Email.tsx. El issue lo archivaba como contraseñas a medio escribir, y mirarlo enseñó que es el caso PEOR — cambiar el correo regenera la clave de recuperación, porque el correo es el salt del que se derivan sus claves (ADR-014), así que la que se ve ahí es la única que ya sirve y la anterior ha dejado de valer. MasterPassword.tsx sí es lo que el issue decía, y se deja sin aviso a propósito con el motivo escrito en el fichero.

Y LA DECISIÓN DE FONDO QUEDA TOMADA Y ESCRITA, no por omisión: la clave se sigue registrando ANTES de enseñarse. Se consideró no registrarla hasta que su dueño confirme que la guardó, y se descartó porque abre un fallo simétrico —quien copia la clave y no confirma se queda con una que el servidor no conoce— y porque contradiría ADR-010, así que necesitaría su propio ADR. El hueco que queda está dicho en RecoveryKey.tsx y no tapado: cerrar la pestaña o un cierre del navegador siguen dejando una cuenta que afirma tener clave, y ahí no hay aviso posible.

EL BUSCADOR YA NO SE PIERDE AL RECORRER LA LISTA, en el 351, y NO se resolvió solo al virtualizar: con 370 entradas la página sigue midiendo 27.532 píxeles, así que a 10.000 de scroll no se veía ni el buscador ni la cabecera. Ahora las dos cosas se quedan arriba. Cuesta 156 píxeles de pantalla en móvil, un 18 por ciento, y ese número está medido y dicho por si en uso real estorba.
...
LAS RUTAS DE LA SPA YA ESTÁN TODAS EN INGLÉS, en el 356: unlock, recover, email, master-password y recovery-key, además de login y register que ya lo estaban. Estaban a medias desde que existen, dos en inglés y cinco en español, que es lo peor de las dos opciones. La excepción está escrita en CLAUDE.md CON SU MOTIVO, que es la mitad que impide que la próxima sesión las traduzca de vuelta por seguir la regla.

LOS TEXTOS DE LA INTERFAZ NO CAMBIARON, comprobado comparando el volcado de ui-text.mjs: las únicas diferencias son las rutas. La pantalla de /recovery-key se sigue titulando Clave de recuperación y el fichero que descarga se sigue llamando evault-clave-de-recuperacion.txt, porque ese nombre sí lo lee una persona. Y NO hay redirecciones desde las rutas viejas: caen en la vault por el catch-all que ya existía.

EL COMPROBADOR DE IDIOMA TENÍA UN AGUJERO Y LA ITERACIÓN 10 SE CERRÓ SIN SABERLO, arreglado en el 366. Solo leía líneas que EMPIEZAN por marcador de comentario, así que los comentarios JSX —que empiezan por llave— y las continuaciones de cualquier bloque escrito sin asteriscos no los miraba nunca: 196 líneas en 16 ficheros, y NUEVE de ellas seguían en español. Sobrevivieron a la conversión por ser invisibles a lo que la declaró terminada.

AHORA LLEVA ESTADO DE BLOQUE y el árbol sale limpio de verdad. Los falsos positivos siguen en CERO sobre 385 líneas inglesas, medido, y el corpus inglés creció de 333 a 385. La frase de CLAUDE.md que decía que la prosa española pegada a código es un descuido y no una zona pendiente queda corregida: no fue cierta hasta el 27 de agosto.

Y UN LÍMITE QUE SIGUE Y AHORA ES EL ÚNICO: en modo diff el estado empieza de cero en cada hunk, así que una línea añadida dentro de un bloque cuya apertura el diff no enseña no se ve. No puede verse, y equivocarse hacia no reportar es la dirección segura. El CI usa --all, que lee ficheros enteros y no tiene esa laguna.
