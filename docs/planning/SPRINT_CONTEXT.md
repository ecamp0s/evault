...
DEL BLOQUE 1 YA ESTÁ EL 303: el aviso de bloqueo dice lo que se va a perder, y SOLO cuando hay algo que perder —decirlo siempre entrenaría a saltarse la frase justo el día que es verdad—. Hay un segundo aviso después del bloqueo que se queda hasta que se descarta, y esa duración no es de gusto: esto salta porque no había nadie delante. Verificado en navegador real con el caso 7 nuevo de verify-auto-lock.mjs, leyendo el texto de la pantalla a los 14,8 minutos. NO se guarda borrador y queda escrito por qué: sería contenido de la vault cifrado con una clave que se acaba de descartar.
