...
Issue 97, los identificadores del código están en dos idiomas: la API en inglés y el frontend en español. La convención ya está escrita en CLAUDE.md y rige para todo lo nuevo; lo que falta es migrar lo anterior, por capas para que cada PR sea revisable. Cuidado al hacerlo con los campos del contrato de la API y con el nombre del store de localStorage, que es evault.sesion: son cadenas y no símbolos, así que un renombrado los rompe sin que el compilador se entere. Lo mismo vale para las claves de config/throttling.php, que además viajan a variables de entorno y por tanto al .env de cualquier despliegue, que es el único sitio que no se actualiza leyendo un diff. Entra en la Iteración 4 troceado en cinco issues, del 115 al 119, uno por capa. El 115, lib/vault, quedó hecho el 3 de agosto de 2026, y dejó un aviso que vale para los cuatro que faltan: los campos nombre, usuario, password, url y notas NO se traducen nunca. No son identificadores, son el formato del blob: ContenidoDeItem se serializa con JSON.stringify y se cifra tal cual, así que renombrarlos dejaría ilegible todo lo ya guardado sin que el compilador dijera nada y sin forma de repararlo. Está avisado en types.ts y en schema.ts. El 116, lib, se cerró el mismo día y dejó dos avisos propios. Uno: además del nombre del store de localStorage, que es evault.sesion y no se toca, la propiedad de dentro también estaba persistida —se llamaba usuarioRecordado— así que renombrarla habría dejado a quien ya usaba la aplicación con un login en blanco en vez de su pantalla de bloqueo. Se resuelve en el merge del store y NO con el par version/migrate, que es lo que uno escribe primero: zustand solo llama a migrate cuando el valor guardado trae una version numérica, y lo guardado no la trae, así que la migración no llegaba a ejecutarse nunca. Dos: registrar no puede pasar a register en el frontend, porque colisiona con el register de react-hook-form; se llama signUp. El 117, components, añadió una tercera de la misma familia: los guards escriben en el state de react-router la ruta de la que vienen, bajo una clave que Login y Desbloquear leen con un cast. Ni el compilador ni ningún test la vigilaban, así que renombrarla en un sitio y no en los otros habría roto en silencio la promesa de volver a donde estabas. Ahora tiene test. El 118, pages, era la capa con casi todo el texto de interfaz y confirmó que la protección hay que hacerla por capas: no basta con apartar comentarios y cadenas, hacen falta también el texto JSX, los fragmentos partidos por una interpolación —Ninguna entrada coincide con «{query}» se rompió justo ahí— y los regex literales de los tests, que llevan textos de interfaz sin comillas. La comprobación que sí sirve es comparar todo el texto visible antes y después, y compararlo con un extractor que entienda esos fragmentos. La otra lección del 115 es de método: npx tsc --noEmit NO comprueba nada en este proyecto, porque tsconfig.json tiene files vacío y usa project references; la comprobación de tipos de verdad es npm run build.
