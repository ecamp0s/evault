...
El issue 182, la quinta capa del renombrado: pages/auth, con 15 identificadores. Register.tsx contiene el aviso de que perder la contraseña maestra es perderlo todo, y ese aviso tiene test propio: si el texto cambia, el test falla. Eso es una red, no un estorbo. La capa se mide con ./scripts/check-identifiers.py --area web y el texto visible con node scripts/identifiers/dump-ui-text.mjs.
